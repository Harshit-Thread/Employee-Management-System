...
<head>
  ...
  <title>Today Employee Attendance Details</title>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800 mb-2">Today Employee Attendance Records</h1>
          <a href="checkoverallattendance.php"><button type="button" class="btn btn-primary mb-2">Check overall Attendance</button></a>
...
<p style="font-size:16px; color:red" align="center"> <?php if($msg){
 ...
 <table class="table table-striped table-hover mt-3">
  ...
  $qry = "SELECT * FROM `employee`.`empattendace`;";
  $res = $con->query($qry);
  while($data = mysqli_fetch_assoc($res)){

  $qry = "select EmpFname,EmpLName from employeedetail where `ID`=".$data['EmpID'].";";
  $res1 = $con->query($qry);
...
if($data['Date'] == $today_date){
  echo"
    <tr>
      <td class='row-data'>$k</td>
      <td class='row-data'>".$data['EmpID']."</td>
      <td class='row-data'>$empname</td>
      <td class='row-data'>".$data['Date']."</td>
      <td class='row-data'>";
      if($data['Date'] !=NULL){
        echo "Present";
      }
