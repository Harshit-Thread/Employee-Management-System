...
<head>
  ...
  <title>Employees Bank Details</title>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">Employee Bank Records</h1>
...
<p style="font-size:16px; color:red" align="center"> <?php if($msg){
 ...
 <table class="table table-striped table-hover mt-3">
  ...
  <?php 
  $id = $_SESSION['uid'];
  $qry = "SELECT * FROM `employee`.`empbankdetails`;";
  $res = $con->query($qry);
  while($data = mysqli_fetch_assoc($res)){
  echo"
    <tr>
      <td class='row-data'>".$data['Holder Name']."</td>
      <td class='row-data'>".$data['Account Number']."</td>
      <td class='row-data'>".$data['Bank Name']."</td>
      <td class='row-data'>".$data['IFSC']."</td>
      <td class='row-data'>".$data['Branch Name']."</td>
    </tr>";
  }
    ?>
